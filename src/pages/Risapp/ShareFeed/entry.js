...
class ShareFeed extends PureComponent {
  ...
  componentDidMount() {
    ...
    //埋点 event: lucky_show
    window.gtag('event', 'h5_share_page_show', { action: this.props.location.query.feed_id });
  ...
  toDownload(type) {
    //埋点 event: lucky_show
    window.gtag('event', 'h5_feedshare_download_click', { label: type, action: this.props.location.query.feed_id });
    window.open(
  ...
  shareMore(type) {
    ...
    //埋点 event: lucky_show
    window.gtag('event', 'h5_share_content_click', { action: this.props.location.query.feed_id, label: type });
    switch (type) {
      case 0:
  ...
  render() {
    ...
    const { risApp: { risData } } = this.props;
    const { user, open, swiperData } = this.state;
    console.log(risData);
    ...
    return (
      <div className={styles.risApp} ref={(ref) => this.node = ref}>
        <div className={styles.carousel}>
          ...
          <div className={styles.inviteMore} onClick={() => { this.toDownload('videocover') }}>{formatMessage({ id: 'risApp.open' })}</div>
        </div>
        {
          it ? //map((it, key) =>
